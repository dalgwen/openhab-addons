...
**NOTE:** This README is **BIG**. You probably don't need to read it all.
If you don't bother about understanding everything, and just want to see how to use it, go to the [Examples](#examples) section.
They are self-explanatory and may even be enough for all your needs.
...
**Tip: all your scripts, _including libraries_, can extend the `Java223Script` class.
...
Here, `ItemStateUpdate` is a class defined in the helper-lib.jar.
...
So, by using the right event object for your trigger, such as `ItemStateUpdate` in this example, you don't have to check the documentation to search for how the event parameter you need is named, and you won't miss the parameter because you misspelled it.
...
It will produce this kind of wrapped script:
...
public class BasicExample extends Java223Script {
    public void main() {
        _items.myitem().send(ON); // let there be light on your item named 'myitem'
    }
}
```
...
Put your rules files under the `automation/jsr223` directory and name it with a `.java` extension.
...
The method parameter is a strongly typed library element (`ItemStateUpdate`) and as such, its fields are auto-injected with the right value from the input.
...
public class ItemsAndThingAccessExample extends Java223Script { // <-- take the Java223Script class as a base class
                                                                // for easier access to _items and _things

    public void exec() {
        _items.myLightItem().send(ON); // <-- light on !
        logger.info(_things.zwave_device_2ecfa3a2_node68().getStatus().toString()); // <-- get thing info
        logger.info("Access to the uuid: {}", Things.zwave_device_2ecfa3a2_node68); // by the way, static uuid String also available
    }
}
```
...
Here, for example, we can use the `SMSModemActions` from the `smsmodem` scope (defined by the smsmodem binding).
...
## Full-fledged example without the helper library

**NOTE**: This is the HARD way of creating a rule. Use this only if you want full control or don't want to use the helper library.
...
Instead of using a file, you may want to write a rule in MainUI by using the 'code' tab.
When creating a rule this way, and with the action written in Java, you can access the trigger inputs with the `bindings` variable:
