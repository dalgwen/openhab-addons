...
import org.openhab.binding.musicassistant.internal.MusicAssistantBindingConstants;
import org.openhab.binding.musicassistant.internal.config.MusicAssistantServerConfig;
import org.openhab.binding.musicassistant.internal.discovery.MusicAssistantPlayerDiscoveryService;
import org.openhab.binding.musicassistant.internal.dto.ButtonDTO;
import org.openhab.binding.musicassistant.internal.dto.ButtonDTODeserializer;
import org.openhab.binding.musicassistant.internal.dto.ButtonsDTO;
import org.openhab.binding.musicassistant.internal.dto.StatusResponseDTO;
import org.openhab.binding.musicassistant.internal.model.Favorite;
...
/**
 * Handles connection and event handling to a MusicAssistant Server.
 *
 ...
 */
@NonNullByDefault
public class MusicAssistantServerHandler extends BaseBridgeHandler {
    private final Logger logger = LoggerFactory.getLogger(MusicAssistantServerHandler.class);

    public static final Set<ThingTypeUID> SUPPORTED_THING_TYPES_UIDS = Set.of(MUSICASSISTANTSERVER_THING_TYPE);
    ...
    private static final @Nullable String NEW_LINE = System.getProperty("line.separator");
    ...
    private List<MusicAssistantPlayerEventListener> squeezeBoxPlayerListeners = Collections
            .synchronizedList(new ArrayList<>());

    private Map<String, MusicAssistantPlayer> players = Collections.synchronizedMap(new HashMap<>());
    ...
    public MusicAssistantServerHandler(Bridge bridge) {
        super(bridge);
    }
    ...
    public Collection<Class<? extends ThingHandlerService>> getServices() {
        return Set.of(MusicAssistantPlayerDiscoveryService.class);
    }
    ...
    public void addPlaylistItem(String mac, String url, @Nullable String title) {
    ...
    /**
     * Connects to a MusicAssistant Server
     */
    private void connect() {
        logger.trace("attempting to get a connection to the server");
        disconnect();
        MusicAssistantServerConfig config = getConfigAs(MusicAssistantServerConfig.class);
        ...
        try {
            SqueezeServerListener listener = this.listener = new SqueezeServerListener();
            listener.start();
    ...
    /**
     * Disconnects from a MusicAssistant Server
     */
    private void disconnect() {
        SqueezeServerListener listener = this.listener;
        Socket clientSocket = this.clientSocket;
        try {
            if (listener != null) {
                listener.terminate();
    ...
    private class SqueezeServerListener extends Thread {
        ...
        public SqueezeServerListener() {
            super(String.format("OH-binding-%s-%s", MusicAssistantBindingConstants.BINDING_ID, "ServerListener"));
        }
        ...
        public void run() {
            ...
            try {
                Socket clientSocket = MusicAssistantServerHandler.this.clientSocket;
                if (clientSocket == null) {
                    throw new IllegalStateException("Client socket is null");
                }
        ...
        private void handlePlayersList(String message) {
            ...
            for (String playerParams : playersList) {
                ...
                String[] playParamsArray = playerParams.split("\\s");
                final Map<String, String> keysAndValues = decodeKeyValueResponse(playParamsArray).stream()
                ...
                final MusicAssistantPlayer player = new MusicAssistantPlayer(macAddress, keysAndValues.get("name"),
            ...
            for (final MusicAssistantPlayer player : players.values()) {
        ...
        private String constructCoverArtUrl(String mac, boolean coverart, @Nullable String coverid,
        ...
        private void handleFavorites(String message) {
            ...
            for (KeyValue entry : decodeKeyValueResponse(messageParts)) {
                ...
                // Favorite name
                else if ("name".equals(entry.key) && f != null) {
                    f.name = entry.value;
        ...
        private ScheduledFuture<?> scheduleRequestFavorites() {
            ...
            return scheduler.schedule(MusicAssistantServerHandler.this::requestFavorites, 3L, TimeUnit.SECONDS);
        }

        private void updateCustomButtons(final String mac) {
            String jsonRpcUrl = MusicAssistantServerHandler.this.jsonRpcUrl;
            if (jsonRpcUrl == null) {
    ...
    /**
     * Interface to allow us to pass function call-backs to MusicAssistant Player
     * Event Listeners
     *
    ...
    interface PlayerUpdateEvent {
        void updateListener(MusicAssistantPlayerEventListener listener);
    }
    ...
    private void updatePlayer(PlayerUpdateEvent event) {
        ...
        synchronized (squeezeBoxPlayerListeners) {
            for (MusicAssistantPlayerEventListener listener : squeezeBoxPlayerListeners) {
        ...
        for (Thing thing : things) {
            ...
            if (handler instanceof MusicAssistantPlayerEventListener playerEventListener
                    && !squeezeBoxPlayerListeners.contains(playerEventListener)) {
     ...
     * @return
     */
    public boolean registerMusicAssistantPlayerListener(MusicAssistantPlayerEventListener squeezeBoxPlayerListener) {
     ...
     * @return
     */
    public boolean unregisterMusicAssistantPlayerListener(MusicAssistantPlayerEventListener squeezeBoxPlayerListener) {
    ...
    private void cancelReconnect() {
        Future<?> reconnectFuture = this.reconnectFuture;
