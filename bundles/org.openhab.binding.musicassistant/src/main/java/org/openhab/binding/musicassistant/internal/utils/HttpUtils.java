...
public class HttpUtils {
     ...
     * @throws MusicAssistantCommunicationException
     * @throws MusicAssistantNotAuthorizedException
     */
    public static String post(String url, String postData)
            throws MusicAssistantNotAuthorizedException, MusicAssistantCommunicationException {
        if (!client.isStarted()) {
            try {
                client.start();
            } catch (Exception e) {
                throw new MusicAssistantCommunicationException("Jetty http client exception: " + e.getMessage());
            }
        }
        ...
        if (statusCode == HttpStatus.UNAUTHORIZED_401) {
            ...
            throw new MusicAssistantNotAuthorizedException("Unauthorized: " + statusLine);
        }

        if (statusCode != HttpStatus.OK_200) {
            String statusLine = response.getStatus() + " " + response.getReason();
            logger.error("HTTP POST method failed: {}", statusLine);
            throw new MusicAssistantCommunicationException("Http post to server failed: " + statusLine);
        }
