...
/**
 * The {@link MusicAssistantPlayerState} is responsible for saving the state of a player.
 *
 * @author Mark Hilbush - Initial contribution
 * @author Patrik Gfeller - Moved class to its own file.
 */
@NonNullByDefault
class MusicAssistantPlayerState {
    private final Logger logger = LoggerFactory.getLogger(MusicAssistantPlayerState.class);
    ...
    private MusicAssistantPlayerHandler playerHandler;

    public MusicAssistantPlayerState(MusicAssistantPlayerHandler playerHandler) {
    ...
    /***
     * Return the player state as {@link MusicAssistantPlayerPlayState}
     *
     * @return {@link MusicAssistantPlayerPlayState}
     */
    MusicAssistantPlayerPlayState getPlayState() {
        if (!isPlaying() && !isStopped()) {
            return MusicAssistantPlayerPlayState.PAUSE;
        }

        if (isPlaying()) {
            return MusicAssistantPlayerPlayState.PLAY;
        }

        return MusicAssistantPlayerPlayState.STOP;
    }
}
