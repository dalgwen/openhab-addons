...
import org.openhab.binding.musicassistant.internal.MusicAssistantStateDescriptionOptionsProvider;
import org.openhab.binding.musicassistant.internal.config.MusicAssistantPlayerConfig;
...
/**
 * The {@link MusicAssistantPlayerHandler} is responsible for handling states, which
 ...
 */
@NonNullByDefault
public class MusicAssistantPlayerHandler extends BaseThingHandler implements MusicAssistantPlayerEventListener {
    private final Logger logger = LoggerFactory.getLogger(MusicAssistantPlayerHandler.class);

    public static final Set<ThingTypeUID> SUPPORTED_THING_TYPES_UIDS = Set.of(MUSICASSISTANTPLAYER_THING_TYPE);
     ...
     */
    private @Nullable MusicAssistantServerHandler squeezeBoxServerHandler;
    ...
    private MusicAssistantStateDescriptionOptionsProvider stateDescriptionProvider;
    ...
    /**
     * Creates MusicAssistant Player Handler
     *
     * @param thing
     * @param callbackUrl
     * @param stateDescriptionProvider
     */
    public MusicAssistantPlayerHandler(Thing thing, @Nullable String callbackUrl,
            MusicAssistantStateDescriptionOptionsProvider stateDescriptionProvider) {
        super(thing);
    ...
    public void initialize() {
        mac = getConfig().as(MusicAssistantPlayerConfig.class).mac;
        if (mac.isBlank()) {
        ...
        MusicAssistantServerHandler squeezeBoxServerHandler = this.squeezeBoxServerHandler;
    ...
    private void updateThingStatus() {
        ...
        if (bridge != null) {
            squeezeBoxServerHandler = (MusicAssistantServerHandler) bridge.getHandler();
     ...
     * @return
     */
    public @Nullable MusicAssistantServerHandler getMusicAssistantServerHandler() {
    ...
    public int getNotificationTimeout() {
        return getConfigAs(MusicAssistantPlayerConfig.class).notificationTimeout;
    }
    ...
    public PercentType getNotificationSoundVolume() {
        ...
        Integer configNotificationSoundVolume = getConfigAs(MusicAssistantPlayerConfig.class).notificationVolume;
    ...
    public void playNotificationSoundURI(StringType uri) {
        ...
        try (MusicAssistantNotificationPlayer notificationPlayer = new MusicAssistantNotificationPlayer(this,
                squeezeBoxServerHandler, uri)) {
            notificationPlayer.play();
        } catch (InterruptedException e) {
            logger.warn("Notification playback was interrupted", e);
        } catch (MusicAssistantTimeoutException e) {
            logger.debug("MusicAssistantTimeoutException during notification: {}", e.getMessage());
        } finally {
