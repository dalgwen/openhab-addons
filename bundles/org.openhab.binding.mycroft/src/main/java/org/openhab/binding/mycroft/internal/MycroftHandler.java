...
public class MycroftHandler extends BaseThingHandler implements MycroftConnectionListener {
        ...
        if (!isStartingWebSocket.compareAndExchange(false, true)) {
            try {
                if (connection.isConnected()) {
                    ...
                    if (sometimesSendVolumeRequest >= 3) { // arbitrary one one three times
    ...
    public void initialize() {
        ...
        logger.debug("Start initializing mycroft 2 {}", thing.getUID());
        System.out.println("Start initializing mycroft 3 thing.getUID()");
        ...
        registerChannel(new FullMessageChannel(this, config.messageTypes));
    ...
    public void connectionEstablished() {
        stopTimer();
        logger.info("Mycroft thing {} is online", thing.getUID());
