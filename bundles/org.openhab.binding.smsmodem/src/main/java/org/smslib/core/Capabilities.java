    ...
    public boolean matches(OutboundMessage message) {
        ...
        for (Caps c : Caps.values()) {
            if (c == Caps.CanSetSenderId) {
                if (message.getOriginatorAddress().getAddress().length() > 0) {
                    bs.set(c.ordinal());
                }
            } else if (c == Caps.CanRequestDeliveryStatus) {
                if (message.getRequestDeliveryReport()) {
                    bs.set(c.ordinal());
                }
            } else if (c == Caps.CanSplitMessages) {
                if (message.getPayload().isMultipart()) {
                    bs.set(c.ordinal());
                }
                ...
                if (message.getEncoding() == Encoding.EncUcs2) {
                    bs.set(c.ordinal());
                }
            } else if (c == Caps.CanSendBinaryMessage) {
                if ((message instanceof OutboundBinaryMessage) || (message.getEncoding() == Encoding.Enc8)) {
                    bs.set(c.ordinal());
                }
            } else if (c == Caps.CanSendUnicodeMessage) {
                if (message.getEncoding() == Encoding.EncUcs2) {
                ...
                if (message instanceof OutboundWapMessage) {
                    bs.set(c.ordinal());
                }
            } else if (c == Caps.CanSendPortInfo) {
                if (message.getSourcePort() != -1 || message.getDestinationPort() != -1) {
                    bs.set(c.ordinal());
                }
            }
        }
