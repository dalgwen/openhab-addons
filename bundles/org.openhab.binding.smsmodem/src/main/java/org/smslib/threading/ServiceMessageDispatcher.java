    ...
    public void run() {
        ...
        while (!this.shouldCancel) {
            try {
                ...
                if (pollNow) {
                    OutboundMessage message = this.messageQueue.get(Settings.serviceDispatcherQueueTimeout,
    ...
    private int getNoOfStartedGateways() {
        try {
            ...
            for (String gatewayId : Service.getInstance().getGatewayIDs()) {
                if (Service.getInstance().getGatewayById(gatewayId).getStatus() == Status.Started) {
                    count++;
                }
            }
            return count;
        } catch (Exception e) {
            logger.debug("Gateway list modified, re-testing...", e);
