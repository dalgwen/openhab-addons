...
package org.smslib.pduUtils.wappush;
...
public class WapSiUserDataGenerator {
    ...
    // 1 Visit GMail now! 2 now! now! now! 3 now! now! now! 4 now! now! now! 5 now! now! now! 6 now! now! now! 7 now!
    // now! now! 8 now! now! now! 9 now! now! now!
    // dcs udl udhl port concat wsp header si data href created expires signal indicationText
    // PDU1: 0051000C91360936310732 00 F5 FF 8B 0C 05 04 0B84 23F0 08 04 7A5B 02 01 29 06 06 03 AE 81 EA 8D CA 02056A00
    // 45 C6 0E036D61696C2E676F6F676C6500850300 08 01 03
    ...
    // 0051000C91360936310732 00 F4 FF 8C 0B 05 04 0B84 23F0 00 03 A8 02 01 01 06 06 03 AE 81 EA 8D CA 02056A00 45 C6
    // 0E036D61696C2E676F6F676C6500850300 0A C30720080523170534 10 C30720080524170534 08 01 03
    // 3120566973697420474D61696C206E6F77212032206E6F7721206E6F7721206E6F77212033206E6F7721206E6F7721206E6F77212034206E6F7721206E6F7721206E6F77212035206E
    ...
    // 0051000C91360936310732 00 F4 FF 60 0B 05 04 0B84 23F0 00 03 A8 02 02 6F7721206E6F7721206E6F77212036206E6F
    // 7721206E6F7721206E6F77212037206E6F7721206E6F7721206E6F77212038206E6F7721206E6F7721206E6F77212039206E6F7721206E6F7721206E6F7721000101
    ...
    private void writeHrefAttribute(String url) throws Exception {
        ...
        if ((myUrl == null) || (myUrl.trim().equals(""))) {
            throw new RuntimeException("Invalid URL: '" + myUrl + "'");
        }
        ...
        for (String protocol : WapPushUtils.getProtocols()) {
            if (myUrl.startsWith(protocol)) {
                ...
                protocolFound = true;
                myUrl = myUrl.substring(protocol.length());
                break;
            }
        }
        ...
        for (int i = 0, lastPosition = 0; i < myUrl.length(); i++) {
            for (String domain : WapPushUtils.getDomains()) {
                // if next characters match the domain
                if (i + domain.length() > myUrl.length()) {
                    // write remainder
                    String currentPortion = myUrl.substring(lastPosition, myUrl.length());
                    this.baos.write(currentPortion.getBytes("UTF-8"));
                    // make everything end
                    i = i + domain.length();
                    break;
                }
                if (myUrl.substring(i, i + domain.length()).equalsIgnoreCase(domain)) {
                    // write current string portion of the url
                    if (lastPosition < i) {
                        String currentPortion = myUrl.substring(lastPosition, i);
                        this.baos.write(currentPortion.getBytes("UTF-8"));
                    }
                    // write domain byte
