...
public abstract class AbstractGateway {
    ...
    public boolean wasActiveSecondsAgo(long secondsAgo) {
        ...
        if (lastUpdated == null || lastCommIn == null) {
            return true;
        }
        ...
        try {
            if ((getStatus() == Status.Stopped) || (getStatus() == Status.Error)) {
                try {
                    ...
                    logger.debug(String.format("Starting gateway: %s", toShortString()));
            ...
            try {
                if ((getStatus() == Status.Started) || (getStatus() == Status.Error)) {
                    try {
                        ...
                        logger.debug(String.format("Stopping gateway: %s", toShortString()));
                        for (int i = 0; i < this.gatewayMessageDispatchers.length; i++) {
